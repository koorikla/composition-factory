...
test('an object parameter persists and offers itself for map fields', async ({ page, request }) => {
  ...
  // the map-typed field's bind quick-pick lists it (tags is a map; the
  // field-list row, since the set field also has an essentials row, CF-470)
  await page.click('.node[data-id="dead-letter"] .node-h')
  await page.click('#fseg button[data-f="all"]')
  await page.click('#insp .fld button[data-m="w"][data-path="tags"]')
  const pick = page.locator('#insp .fld select[data-wire="tags"]')
