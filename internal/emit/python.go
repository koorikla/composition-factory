...
func pythonTemplateBody(b *blueprint.Blueprint, crds []schema.CRD) (string, error) {
	...
	for _, r := range b.Spec.Resources {
		...
		// Spec
		if crd.Native {
			sb.WriteString(fmt.Sprintf("%s\"spec\": {\n", inner))
			...
			// forProvider
			sb.WriteString(fmt.Sprintf("%s\"forProvider\": {\n", specInner))
...
func translateForEachToPython(forEach string) string {
	...
	if strings.HasPrefix(forEach, "resources.") {
		...
		var sb strings.Builder
		sb.WriteString(fmt.Sprintf("range(int(ocds.get(%q, {}).get(\"resource\", {}).get(\"status\", {}).get(\"atProvider\", {})", res))
		for i, p := range parts {
			if i == len(parts)-1 {
