...
// The v1 union ruling for conventions x native Kubernetes kinds: refused
// outright. A convention fills top-level forProvider fields, and a native
// object has no forProvider — its own top-level fields are structural (a
...
// The parallel ruling for template: fields on a native resource itself: the
// template call's output re-indents to the fixed forProvider column
// (templateFieldNindent in internal/emit), which a native field at an
// arbitrary nesting depth breaks — structurally broken YAML in the rendered
// document. Refused at the source.
func TestValidateRefusesTemplateFieldOnNativeResource(t *testing.T) {
	b := templatedBlueprint(func(b *Blueprint) {
	...
	err := b.Validate()
	if err == nil {
		t.Fatal("Validate accepted a template: field on a native resource")
	}
	for _, want := range []string{`"web"`, "template", NativeProvider} {
