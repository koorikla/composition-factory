...
- **One Merge at a Time**: Many drivers may run at once. They are described in
  ...
  (skill in `.claude/skills/oracle/`); [`docs/routines/refiner.md`](docs/routines/refiner.md) turns verified issues into
  brief-ready briefs, and [`docs/routines/janitor.md`](docs/routines/janitor.md) reclaims dead worktrees, branches, labels and
  gate slots. Neither of those two claims, dispatches or lands. Only `scripts/driver/land.sh` pushes `main`, one landing at a time: it holds the merge lock for its whole run, through the landing's CI and any revert. All other agents work in isolated topic branch worktrees, push only their own topic branch, and hand back on the issue.
...
- **Post-Merge CI Check**: `scripts/driver/land.sh` watches the landing's `ci` run to completion and reverts the landing if it is red — a landing is not done until CI is green, and the driver acts on the result line `land.sh` prints. Two of the five jobs (`cluster`, `e2e`) exercise a real kind cluster and a browser and cannot be reproduced by unit tests, so a locally-green tree says nothing about them. Two reruns exist, and they follow different rules. **The landing's own run** is rerun at most once, and only when `e2e` is the single non-passing job and it concluded `failure`: its canvas drag tests are flaky in CI. No other job, `acceptance` included, is ever rerun there, and no landing run is rerun twice. **`main`'s own run**, which `land.sh` reads before it lands anything, is rerun by GitHub's record rather than per call: on attempt 1 (or an unknown attempt) a red or `cancelled` run is rerun once — `--failed`, or the whole run when it was cancelled — and from attempt 2 on it is rerun only if that attempt has sat unchanged longer than `CF_LAND_MAIN_RERUN_AFTER_SEC` (default 3600). Otherwise `land.sh` refuses with `MAIN-RED` and lands nothing but `severity:P0`, which bypasses the refusal so a red `main` never blocks its own fix. No agent other than `land.sh` judges `main`'s CI. Subagents working in topic-branch worktrees are exempt — they hand back branches and the driver lands them through `land.sh`.
