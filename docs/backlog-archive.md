...
- [x] **CF-081 — Nothing renders one blueprint through every engine and compares composed
      ...
      blueprint across all three engines (go-templating, KCL, and Python) through their
      real function images, decodes each composed resource into structured documents,
      and deeply diffs them across engines via `cmp.Diff` with `t.Errorf`. Verifies identical
      composed resource sets, kinds, and apiVersions, comparing normalized specs and metadata
      while documenting ignored render artifacts (`metadata.name` suffix generated by Crossplane).
      Also fixed Python status wire quoting so unobserved status references evaluate to `None`
      and are cleanly omitted from desired resources.
      — completed 2026-09-09
...
- [x] **CF-074 — A status wire into `fields:` is interpolated unquoted, so a string status
      ...
      — completed 2026-09-09
- [x] **CF-080 — The `k8s-workload` starter composes a ConfigMap the API server rejects under
      ...
      — completed 2026-09-09
- [x] **CF-082 — A provider dropped from `spec.sources` can never be re-added: the add
...
- [x] **CF-085 — The canvas replaces every card element when web fonts finish loading,
      ...
      unaccounted for. The fix makes `render()` reconcile card elements by `data-id` in
      place instead of destroying and replacing outer nodes with `innerHTML = h`.
      — completed 2026-09-09
